...
export const ImageCard = (props: ImageCardProps) => {
    let realSize = (parseInt(props.size) / 1000000).toFixed(2)
    let nameAndTag = props.tags[0].split(":", 2)
    const router = useRouter()
    let admin: boolean = router.query.admin == "true";
        ...
        await fetch('/api/containers/create', {
            ...
            .then(data => {
                if (data.status === "success") {
                    router.push("/containers?admin=" + String(admin));
                } else {
                    router.push("/");
                }
            })
    ...
    return (
        <div className={styles.image_card}>
            ...
            <button className={styles.button} onClick={createContainer} disabled={!admin}>
                New instance
            </button>
        </div>
    );
